      ...
      <div className="space-y-8">
        ...
        <section>
          ...
          <ul className="list-disc pl-6 space-y-1">
            ...
            <li><strong>Do Now</strong> sends your stated health objective, your age group, medical conditions, injuries, today&apos;s journal entries, recent journal context, and any active health concerns to help you choose between options you are weighing. No names, exact biometrics, or account identifiers are transmitted.</li>
            <li><strong>Body Age</strong> sends your anonymous health profile (age, sex, height, weight, BMI, and ethnicity, a sensitive data category under PDPA transmitted only with your explicit per-session consent), vital signs (resting heart rate, HRV, blood oxygen, respiratory rate, VO₂ max), blood pressure, body composition (body fat percentage, visceral fat, BMI), available lab biomarkers from scanned documents (such as HbA1c, total cholesterol, triglycerides, eGFR, creatinine, haemoglobin, TSH, liver enzymes, uric acid, ferritin, and vitamin D levels), Fitzpatrick skin type, bone and joint conditions, smoking status, sleep quality summary, medical conditions, lifestyle factors, and a 30-day journal activity summary to estimate your biological age. All data is pseudonymised using a daily-rotating identifier; no names, exact birth dates, or account identifiers are transmitted.</li>
            <li><strong>Body Age.</strong> When you use the Body Age feature, your chronological age (derived from the birth year in your profile) is transmitted to our third-party AI providers as an input to the biological-age estimate. It is sent only for that computation and is not used to identify you.</li>
            ...
            <li><strong>Mistral AI (France):</strong> Mistral is an available AI provider for most Flair features: Do Now, Blueprint, Weekly Report, Body Age, Inspire Me, journal analysis, and Document Scanner. When you select Mistral as your AI provider for any of these features, it receives the data required for that feature, formatted according to the consent form you approved: your health context, journal entries, medical conditions, vital signs, and biomarker data (if available from scanned documents).</li>
            <li><strong>Mistral Data Handling &amp; Training:</strong> Mistral does not use your health data, journal entries, or photos for model training. Requests reach Mistral through Flair&apos;s server, so Mistral never receives your IP address, and Flair is accountless, so no account identity exists to share. For details on how Mistral handles your data, see their <a href="https://legal.mistral.ai/terms/privacy-policy" className="text-blue-600 dark:text-blue-400 hover:underline" target="_blank" rel="noopener noreferrer">privacy policy</a>.</li>
            <li><strong>Pseudonymous Telemetry:</strong> Your device generates a pseudonymous identifier by hashing a random per-install value with the current date, and includes it in requests to Flair&apos;s routing layer. It is never forwarded to any AI provider. The providers that process your request receive no identifier of any kind. We use it to enforce rate limits, and we retain it alongside an operational record of each request covering which feature was called, when, how long it took, how many tokens were used, and which model answered. When you use Refresh Insights, this record also includes the brand categories of any connected wearables or health apps detected in Apple Health (for example, &quot;Garmin&quot; or &quot;Oura&quot;), never the device model or firmware version. These records contain no journal content, no health values, and no IP address. They do show which features a given day&apos;s identifier used, so they are a record of activity rather than an anonymous count. Because the identifier changes every day, that activity cannot be linked from one day to the next. The random value it derives from never leaves your device. All telemetry is subject to a 90 day retention limit. Separately, Body Age assessments contribute a pseudonymised age delta (estimated biological age minus chronological age) to a weekly aggregate used for model performance monitoring; this aggregate carries no identifier of any kind and cannot be linked to the record described above.</li>

...
tag (`policy-v1` through `policy-v36`) on the corresponding commit, as a
cheaper, more durable handle than memorizing a hash. Tags are pushed to the
remote and are covered by the same no-rewrite rule as the commits they
point to.
